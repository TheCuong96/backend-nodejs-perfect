...
const uploadSingleFiles = async (filesArray) => {
    const dirPath = path.join(process.cwd(), "/src/public/images/uploads"); // tạo 1 đường dẫn đến thư mục có tên pictures, dùng process.cwd() trỏ đến src của ta,còn __dirname trỏ đến thư mục mà nó đang đứng,nên sẽ phải fix lại mỗi khi đem đoạn code này đi chỗ khác, nên ta dùng process.cwd() để không cần phải fix lại
